...
def build_bot_router() -> APIRouter:
    """构建机器人账号管理路由"""
    router = APIRouter(prefix="/bots", tags=["WebUI-机器人账号"])
    ...
    async def list_bots() -> dict[str, Any]:
        ...
        return {
            ...
            "count": len(bots),
            "online_count": sum(1 for b in bots if b.bot_id in online_ids),
        }
    ...
    async def set_bot_status(
        ...
        return {"ok": True, "bot_id": bot_id or "all", "status": status}
    ...
    async def bot_detail(bot_id: str) -> dict[str, Any]:
        ...
        if not bot:
            raise HTTPException(status_code=404, detail=f"未找到机器人: {bot_id}")
        online_ids = set(get_bots().keys())
        data = _bot_to_view(bot, online_ids)
        data["available_plugins_list"] = BotConsole.convert_module_format(bot.available_plugins)
        data["block_plugins_list"] = BotConsole.convert_module_format(bot.block_plugins)
        data["available_tasks_list"] = BotConsole.convert_module_format(bot.available_tasks)
        data["block_tasks_list"] = BotConsole.convert_module_format(bot.block_tasks)
        return data
