...
def build_plugin_router() -> APIRouter:
    """构建插件管理路由"""
    router = APIRouter(prefix="/plugins", tags=["WebUI-插件管理"])
    ...
    async def list_plugins(
        ...
        参数:
            ...
            menu_type: 菜单类型过滤
        """
        query = PluginInfo.filter(PluginInfo.plugin_type != PluginType.PARENT)
        if load_status is not None:
            query = query.filter(load_status=load_status)
        if menu_type:
            query = query.filter(menu_type=menu_type)
        plugins = await query.all()
        menu_types = sorted({p.menu_type for p in plugins if p.menu_type})
        return {
    ...
    async def list_plugins_for_bot(bot_id: str) -> dict[str, Any]:
        ...
        for p in plugins:
            ...
            result.append(view)
        return {"bot_id": bot_id, "plugins": result, "count": len(result)}
    ...
    async def toggle_plugin(
        ...
        if not bot_id or not module:
            raise HTTPException(status_code=400, detail="bot_id 与 module 不能为空")
        plugin = await PluginInfo.filter(module=module).first()
        if not plugin:
            raise HTTPException(status_code=404, detail=f"未找到插件: {module}")
        if enabled:
            await BotConsole.enable_plugin(bot_id, module)
        else:
            await BotConsole.disable_plugin(bot_id, module)
        return {"ok": True, "bot_id": bot_id, "module": module, "enabled": enabled}
    ...
    async def toggle_all_plugins(
        ...
        return {"ok": True, "bot_id": bot_id, "enabled": enabled, "scope": "all_plugins"}
