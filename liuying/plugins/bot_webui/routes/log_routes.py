...
_ALLOWED_SUFFIXES = {".log", ".log.1", ".txt"}
"""允许读取的日志文件后缀（粗筛）"""
...
def _is_safe_log_name(name: str) -> bool:
    ...
    if not name or "/" in name or "\\" in name or ".." in name:
        return False
    # 文件名必须形如 *.log 或 error_*.log 等纯文件名
...
def build_log_router() -> APIRouter:
    ...
    Returns:
        APIRouter: 日志查看路由器
    """
    router = APIRouter(prefix="/logs", tags=["本体-日志查看"])
    ...
    async def read_log(name: str, lines: int = 0) -> dict[str, Any]:
        ...
        if not _is_safe_log_name(name):
            raise HTTPException(
                status_code=400, detail="非法日志文件名"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=404, detail="日志文件不存在"
            ) from exc
