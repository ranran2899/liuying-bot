...
class BotInfoUtils:
    ...
    async def get_bot_info(cls, bot: Bot) -> tuple[str, str]:
        ...
        match bot.adapter.get_name():
            case "OneBot V11" | "OneBot V12":
                return await cls._get_onebot_info(bot)
            case "QQ":
                return await cls._get_qq_official_info(bot)
            case _:
                return bot_id, ""

    @classmethod
    async def _get_onebot_info(cls, bot: Bot) -> tuple[str, str]:
        ...
        if bot.adapter.get_name() == "OneBot V11":
            ...
            nickname = (
                self_info.get("user_displayname")
                or self_info.get("user_name")
                or bot_id
            )
        avatar_url = ""
    ...
    @classmethod
    async def _get_qq_official_info(cls, bot: Bot) -> tuple[str, str]:
