...
from liuying.utils.limiters import ConcurrencyLimiter, FreqLimiter, RateLimiter
    ...
    async def dependency(
        ...
        if limiter.check(key):
            if isinstance(limiter, FreqLimiter):
                limiter.start_cd(key, kwargs.get("duration_sec", limiter.default_cd))
            return True
        ...
        await matcher.finish(prompt.format(**format_kwargs))
...
def Cooldown(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(f"Cooldown装饰器中的duration格式错误: {e}") from e
...
def RateLimit(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(f"RateLimit装饰器中的duration格式错误: {e}") from e
