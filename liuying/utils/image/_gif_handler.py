...
class GifHandler:
    ...
    def extract_frames(gif_bytes: bytes) -> list[BuildImage]:
        ...
        return [to_build_image(f) for f in frames]
    ...
    def extract_frame(gif_bytes: bytes, index: int = 0) -> BuildImage:
        ...
        with Image.open(BytesIO(gif_bytes)) as gif:
            ...
            frames = list(_iter_full_frames(gif))
            return to_build_image(frames[index])
    ...
    def build_gif(
        ...
        pil_frames = [to_pil_image(f).convert("RGBA") for f in frames]
    ...
    def add_image_watermark(
        ...
        返回:
            bytes: 处理后的 GIF 二进制数据
        """
        wm_pil = to_pil_image(watermark).convert("RGBA")
        if opacity < 1.0:
...
def _iter_full_frames(gif: Image.Image):
    ...
    for frame in ImageSequence.Iterator(gif):
        ...
        # 应用上一帧的 disposal（method=3 需叠加上一帧完整画面）
        if prev_frame is not None and prev_disposal == 3:
