...
def check_cryptography_installed() -> bool:
    ...
    if not HAS_CRYPTOGRAPHY:
        logger.error(
            "SSL证书功能需要安装 cryptography 库，请运行: pip install cryptography",
            "SSLUtils"
        )
        return False
    return True
...
def generate_self_signed_cert(
    ...
    try:
        ...
        logger.info("正在生成SSL私钥...", "SSLUtils")
        ...
        logger.info("正在生成SSL证书...", "SSLUtils")
        ...
        logger.info("正在保存SSL私钥...", "SSLUtils")
        ...
        logger.info("正在保存SSL证书...", "SSLUtils")
        ...
        logger.error(f"生成SSL证书失败: {e}", "SSLUtils", e=e)
        return False
...
def create_ssl_context(
    ...
    **generate_kwargs
    ...
    if not cert_path.exists() or not key_path.exists():
        if auto_generate:
            logger.info("SSL证书不存在，正在自动生成...", "SSLUtils")
            if not generate_self_signed_cert(cert_path, key_path, **generate_kwargs):
                return None
        else:
            logger.error("SSL证书文件不存在", "SSLUtils")
            return None

    try:
        ssl_context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
        ssl_context.load_cert_chain(str(cert_path), str(key_path))
        logger.info("SSL证书加载成功", "SSLUtils")
        return ssl_context
    except Exception as e:
        logger.error(f"加载SSL证书失败: {e}", "SSLUtils", e=e)
        return None


def get_cert_info(cert_path: str | Path) -> dict | None:
    """获取证书信息
    ...
    try:
        ...
        logger.error(f"读取证书信息失败: {e}", "SSLUtils", e=e)
        return None


def is_cert_valid(cert_path: str | Path) -> bool:
    """检查证书是否有效（未过期）
