...
class FgRequest(Model):
    ...
    @classmethod
    async def approve(cls, bot, id: int):
        """同意请求
    ...
    @classmethod
    async def refused(cls, bot, id: int):
        """拒绝请求
    ...
    async def _handle_request(
        ...
        异常:
            NotFoundError: 未发现请求
        """
        req = await cls.safe_get_or_none(id=id)
        if not req:
            raise NotFoundError
            ...
            if req.request_type == RequestType.FRIEND:
                ...
                if req.flag == "0":
                    await bot.send_private_msg(
                        user_id=req.user_id,
                        message=f"已同意你对的申请群组：{req.group_id}，可以直接手动拉入群组，我会自动同意。",
                    )
                else:
