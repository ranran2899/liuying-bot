...
class UserLevel(Model):
    ...
    async def _fetch_level(
        cls, user_id: str, group_id: str | None = None, bot_id: str | None = None
    ) -> int:
        """
        内部方法：获取指定条件的权限等级
    ...
    async def get_user_level(
        cls, user_id: str, group_id: str | None = None
    ) -> int:
        """
        获取用户权限等级（兼容旧接口）
    ...
    @classmethod
    async def get_bot_level(cls, bot_id: str, user_id: str) -> int:
        """
        获取用户在指定机器人上的权限等级
    ...
    async def check(
        ...
        返回:
            bool: 是否满足权限要求
        """
        return await cls.get_level(user_id, bot_id, group_id) >= level
    ...
    async def check_bot_level(
        cls, bot_id: str, user_id: str, level: int = 0
    ) -> bool:
        """
        ...
        返回:
            bool: 是否满足权限要求
        """
        return await cls._fetch_level(user_id, None, bot_id) >= level
