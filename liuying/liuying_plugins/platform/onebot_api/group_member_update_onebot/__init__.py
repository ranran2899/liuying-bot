...
async def _(bot: Bot, session: Uninfo, arparma: Arparma):
    if gid := session.group.id if session.group else None:
        logger.info("更新qq群组成员信息", arparma.header_result, session=session)
                ...
                if group_list:
                    for group in group_list:
                        try:
                            ...
                            logger.debug("自动更新qq群组成员信息成功...")
                        except Exception as e:
                            logger.error(
                                f"Bot: {bot.self_id} 自动更新qq群组成员信息失败",
                                target=group.group_id,
                                e=e,
                            )
            except Exception as e:
                logger.error(f"Bot: {bot.self_id} 自动更新qq群组成员信息失败", e=e)
            else:
                logger.debug(f"自动 Bot: {bot.self_id} 更新qq群组成员信息成功...")
