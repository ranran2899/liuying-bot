...
from liuying.utils.enum import RequestHandleType
from liuying.utils.message import MessageUtils
from liuying.utils.platform import PlatformUtils
from liuying.utils.utils import FreqLimiter
...
DEFAULT_IMAGE_PATH = IMAGE_PATH / "liuying"
...
class GroupManager:
    ...
    async def __handle_add_group(
        ...
        if group:
            ...
            plugin_list = await PluginInfo.filter(default_status=False).all()
            block_plugin = "".join(
                f"<{plugin.module}," for plugin in plugin_list
            )
            ...
            try:
                if result_msg := base_config.get("message"):
                    await bot.send_group_msg(
                        group_id=group_id, message=result_msg
                    )
                ...
                await (
                    FgRequest.filter(group_id=group_id)
    ...
    async def kick_bot(cls, bot: Bot, group_id: str, operator_id: str):
        ...
        参数:
            ...
            operator_id: 操作员id
        """
        if user := await GroupInfoUser.safe_get_or_none(
