...
from liuying.utils.enum import EventLogType, PluginType, RequestHandleType, RequestType
...
cache = CacheRoot.cache_dict("REQUEST_CACHE", 60)


@friend_req.handle()
async def _(bot: v12Bot | v11Bot, event: FriendRequestEvent, session: Uninfo):
    """处理好友请求"""
    ...
    if base_config.get("AUTO_ADD_FRIEND"):
        ...
        ).where_null("handle_type").update(handle_type=RequestHandleType.EXPIRE)
...
@group_req.handle()
async def _(bot: v12Bot | v11Bot, event: GroupRequestEvent, session: Uninfo):
    """处理群组请求"""
    ...
    if str(event.user_id) in bot.config.superusers or base_config.get("AUTO_ADD_GROUP"):
        ...
        if str(event.user_id) not in bot.config.superusers and base_config.get(
            ...
            # 非超级用户邀请自动加入群组
            nickname = await BotFriend.get_user_name(
                bot.self_id, str(event.user_id)
            )
            f = await FgRequest.create(
