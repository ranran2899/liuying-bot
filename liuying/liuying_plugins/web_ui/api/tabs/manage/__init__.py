...
from liuying.utils.enum import RequestHandleType, RequestType
...
async def _(bot_id: str) -> Result:
    ...
    try:
        ...
        logger.error(f"{router.prefix}/get_group_list 调用错误", command="WebUi", e=e)
...
async def _(group: UpdateGroup) -> Result[str]:
    try:
        ...
        logger.error(f"{router.prefix}/update_group 调用错误", command="WebUi", e=e)
...
async def _(bot_id: str) -> Result[list[Friend]]:
    try:
        ...
        logger.error("调用API错误", command="WebUi", e=e)
        return Result.fail(f"{type(e)}: {e}")
...
async def _() -> Result[ReqResult]:
    try:
        ...
        logger.error(f"{router.prefix}/get_request_list 调用错误", command="WebUi", e=e)
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(param: HandleRequest) -> Result:
    try:
        ...
        logger.error(f"{router.prefix}/refuse_request 调用错误", command="WebUi", e=e)
        return Result.fail(f"{type(e)}: {e}")
...
async def _(param: HandleRequest) -> Result:
    try:
        bot = nonebot.get_bot(param.bot_id)
        if not (req := await FgRequest.filter(id=param.id).first()):
            return Result.warning_("未找到此Id请求...")
        ...
        logger.error(f"{router.prefix}/approve_request 调用错误", command="WebUi", e=e)
        return Result.fail(f"{type(e)}: {e}")
...
async def _(param: LeaveGroup) -> Result:
    try:
        ...
        logger.error(f"{router.prefix}/leave_group 调用错误", command="WebUi", e=e)
        return Result.fail(f"{type(e)}: {e}")
...
async def _(param: DeleteFriend) -> Result:
    try:
        ...
        logger.error(f"{router.prefix}/delete_friend 调用错误", command="WebUi", e=e)
        return Result.fail(f"{type(e)}: {e}")
...
async def _(bot_id: str, user_id: str) -> Result[UserDetail]:
    try:
        ...
        logger.error(
            f"{router.prefix}/get_friend_detail 调用错误", command="WebUi", e=e
        )
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(group_id: str) -> Result[GroupDetail]:
    try:
        ...
        logger.error(f"{router.prefix}/get_group_detail 调用错误", command="WebUi", e=e)
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(param: SendMessageParam) -> Result:
    try:
        ...
        logger.error(f"{router.prefix}/send_message 调用错误", command="WebUi", e=e)
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
