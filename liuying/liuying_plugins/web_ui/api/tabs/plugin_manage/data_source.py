...
from liuying.configs.utils import ConfigGroup
...
class ApiDataSource:
    ...
    async def get_plugin_list(
        ...
        # 始终用 filter() 创建 QueryWrapper，避免直接对模型类调用 .all()
    ...
    async def batch_update_plugins(cls, params: BatchUpdatePlugins) -> dict:
        ...
        for item in params.updates:
            try:
                db_plugin = await DbPluginInfo.filter(module=item.module).first()
                if not db_plugin:
        ...
        if plugins_to_update_other_fields and other_update_fields:
            try:
                await DbPluginInfo.filter().bulk_update(
