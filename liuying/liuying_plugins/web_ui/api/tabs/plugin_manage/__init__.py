...
from liuying.models.plugin_info import PluginInfo as DbPluginInfo
from liuying.utils.enum import BlockType, PluginType
from liuying.utils.log import logger
from liuying.utils.manager import VirtualEnvPackageManager
...
async def _(
    ...
    try:
        ...
        logger.error(f"{router.prefix}/get_plugin_list 调用错误", command="WebUi", e=e)
...
async def _(param: UpdatePlugin) -> Result:
    try:
        ...
        logger.error(f"{router.prefix}/update_plugin 调用错误", command="WebUi", e=e)
...
async def _(param: PluginSwitch) -> Result:
    try:
        ...
        logger.error(f"{router.prefix}/change_switch 调用错误", command="WebUi", e=e)
        return Result.fail(f"{type(e)}: {e}")
...
async def _() -> Result[list[str]]:
    try:
        ...
        logger.error(
            f"{router.prefix}/get_plugin_menu_type 调用错误", command="WebUi", e=e
        )
        return Result.fail(f"{type(e)}: {e}")
...
async def _(module: str) -> Result[PluginDetail]:
    try:
        ...
        logger.error(f"{router.prefix}/get_plugin 调用错误", command="WebUi", e=e)
        return Result.fail(f"{type(e)}: {e}")
...
async def _(
    ...
    try:
        ...
        logger.error(
            f"{router.prefix}/plugins/batch_update 调用错误", command="WebUi", e=e
        )
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(payload: RenameMenuTypePayload) -> Result[str]:
    try:
        ...
        logger.error(
            f"{router.prefix}/menu_type/rename 调用错误", command="WebUi", e=re
        )
        return Result.fail(info=str(re))
    except Exception as e:
        logger.error(
            f"{router.prefix}/menu_type/rename 调用错误", command="WebUi", e=e
        )
...
async def _(payload: InstallDependenciesPayload) -> Result:
    try:
        ...
        logger.error(
            f"{router.prefix}/install_dependencies 调用错误", command="WebUi", e=e
        )
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
