...
async def _() -> Result[list[BotInfo]]:
    try:
        ...
        logger.error(f"{router.prefix}/get_bot_list 调用错误", command="WebUi", e=e)
...
async def _(bot_id: str | None = None) -> Result[QueryChatCallCount]:
    try:
        ...
        logger.error(
            f"{router.prefix}/get_chat_and_call_count 调用错误", command="WebUi", e=e
        )
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(bot_id: str | None = None) -> Result[AllChatAndCallCount]:
    try:
        ...
        logger.error(
            f"{router.prefix}/get_all_chat_and_call_count 调用错误",
            command="WebUi",
            e=e,
        )
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(bot_id: str | None = None) -> Result[ChatCallMonthCount]:
    try:
        ...
        logger.error(
            f"{router.prefix}/get_chat_and_call_month 调用错误", command="WebUi", e=e
        )
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(query: QueryModel) -> Result[BaseResultModel]:
    try:
        ...
        logger.error(f"{router.prefix}/get_connect_log 调用错误", command="WebUi", e=e)
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
