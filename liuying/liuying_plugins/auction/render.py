...
async def render_auction(
    ...
    返回:
        bytes: 图片字节数据
    """
    payload = await _build_payload(user_id, items, page, total_pages, keyword)
    return await render(
        _TEMPLATE_PATH,
        data={"page_type": "listing" if not keyword else "search", "payload": payload},
        user_id=user_id,
        wait=2,
    )


async def render_my_auctions(user_id: str, items: list[dict]) -> bytes:
    """渲染我的拍卖页面
    ...
    item_list = [_extract(item, idx) for idx, item in enumerate(items)]
    ...
    record_list = [
        {
            ...
            "fee": r.get("fee", 0),
            "type": r.get("type", ""),
            "type_label": "购买" if r.get("type") == "buy" else "出售",
            "counterpart": (
                r.get("seller_id", "")
                if r.get("type") == "buy"
                else r.get("buyer_id", "")
