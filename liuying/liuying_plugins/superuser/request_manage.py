...
from liuying.utils.enum import PluginType, RequestHandleType, RequestType
from liuying.utils.exception import NotFoundError
from liuying.utils.image import BuildImage
from liuying.utils.log import logger
from liuying.utils.message import MessageUtils
from liuying.utils.utils import get_user_avatar
...
async def _(
    ...
    handle_id = id.result
    if reply:
        db_data = await FgRequest.filter(message_ids__contains=reply.id).first()
        if not db_data:
    ...
    try:
        match handle_type:
            case RequestHandleType.APPROVE:
                req = await FgRequest.approve(bot, handle_id)
            case RequestHandleType.REFUSED:
                req = await FgRequest.refused(bot, handle_id)
            case RequestHandleType.IGNORE:
...
async def _(
    ...
    if all_request := await FgRequest.filter().where_null("handle_type").all():
...
async def _(
    ...
    match (is_friend.result, is_group.result):
        case (True, _):
            _type = "好友"
            await (
                FgRequest.filter(request_type=RequestType.FRIEND)
                .where_null("handle_type")
                .update(handle_type=RequestHandleType.IGNORE)
            )
        case (_, True):
            _type = "群组"
            await (
                FgRequest.filter(request_type=RequestType.GROUP)
                .where_null("handle_type")
                .update(handle_type=RequestHandleType.IGNORE)
            )
        case _:
