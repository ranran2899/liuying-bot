...
async def _(session: Uninfo, arparma: Arparma, state: T_State):
    ...
    logger.info(f"{s}群认证", arparma.header_result, session=session, target=gid)


@_matcher.assign("del-group")
async def _(bot: Bot, session: Uninfo, arparma: Arparma, group_id: int):
