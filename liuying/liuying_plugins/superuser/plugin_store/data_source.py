...
from liuying.utils.manager.virtual_env_package_manager import VirtualEnvPackageManager
from liuying.utils.plugin_init import PluginInitManager
from liuying.utils.repo_utils import repo_file_manager
from liuying.utils.repo_utils.models import RepoFileInfo, RepoType
from liuying.utils.utils import is_number, win_on_rm_error
...
class StoreManager:
    ...
    async def get_data(cls) -> tuple[list[StorePluginInfo], list[StorePluginInfo]]:
        ...
        返回:
            tuple[list[StorePluginInfo], list[StorePluginInfo]]:
                原生插件信息数据，第三方插件信息数据
        """
        plugins = await repo_file_manager.get_file_content(
            DEFAULT_GITEE_URL, "plugins.json"
        )
        try:
            extra_plugins = await repo_file_manager.get_file_content(
                EXTRA_GITEE_URL, "plugins.json", "index"
            )
            extra_plugin_list = [
            ...
            logger.warning(
                f"获取第三方插件列表失败，将跳过第三方插件: {e}", LOG_COMMAND
            )
            extra_plugin_list = []
        return [
    ...
    def check_version_is_new(
        ...
        返回:
            bool: 商店版本是否与已安装版本一致
        """
        return plugin_info.version == suc_plugin.get(plugin_info.module)

    @classmethod
    async def get_loaded_plugins(cls, *args: str) -> list[tuple[str, ...]]:
        """获取已加载的插件
    ...
    async def get_plugins_info(cls) -> list[BuildImage] | str:
        ...
        suc_plugin = {p[0]: (p[1] or DEFAULT_VERSION) for p in db_plugin_list}
        index = 0
        data_list = []
        extra_data_list = []
    ...
    async def add_plugin(cls, index_or_module: str, source: str | None = None) -> str:
        ...
        参数:
            index_or_module: 插件索引、模块名或名称
            source: 下载源，ali 或 git
        ...
        if plugin_info.github_url is None:
            plugin_info.github_url = DEFAULT_GITEE_URL
    ...
    async def install_plugin_with_repo(
        ...
        参数:
            ...
            source: 源
        """
        repo_type = RepoType.GITEE if is_external else None
        if source == "ali":
        ...
        if is_dir:
            files = await repo_file_manager.list_directory_files(
        ...
        if not is_install_req:
            ...
            await repo_file_manager.download_files(
                github_url,
                [
    ...
    async def remove_plugin(cls, index_or_module: str) -> str:
        ...
        if plugin_info.is_dir:
            shutil.rmtree(path, onexc=win_on_rm_error)
        else:
            path.unlink()
    ...
    async def search_plugin(cls, plugin_name_or_author: str) -> BuildImage | str:
        ...
        filtered_data = [
            (idx, plugin_info)
            for idx, plugin_info in enumerate(all_plugin_list)
        ...
        data_list = [
            ...
            for idx, plugin_info in filtered_data
        ]
        if not data_list:
            return "未找到相关插件..."
        column_name = ["-", "ID", "名称", "简介", "作者", "版本", "类型"]
