...
def _convert_v11_media(
    data_dict: dict[str, Any], media_cls: type[Image | Video]
) -> Image | Video | None:
    """转换V11图片/视频数据为UniSeg段"""
    if data_dict.get("url"):
        return media_cls(url=data_dict["url"])
    if data_dict.get("file"):
        file_val = data_dict["file"]
        if isinstance(file_val, str) and file_val.startswith("base64://"):
            b64_data = file_val[9:]
            raw_bytes = base64.b64decode(b64_data)
            return media_cls(raw=raw_bytes)
        return media_cls(path=file_val)
    return None
...
async def _process_v11_segment(
    ...
    match seg_type:
        ...
        case "image":
            img_seg = _convert_v11_media(data_dict, Image)
            if img_seg:
        ...
        case "video":
            video_seg = _convert_v11_media(data_dict, Video)
            if video_seg:
...
async def _extract_content_from_message(
    ...
    match message_content:
        ...
        case _:
            logger.warning(
                f"[Depth {depth}] 无法处理的输入类型: {input_type_str}", "广播"
            )
            return temp_msg
