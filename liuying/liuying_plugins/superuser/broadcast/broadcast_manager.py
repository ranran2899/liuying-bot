...
class BroadcastManager:
    ...
    async def get_all_groups(cls, bot: Bot) -> tuple[list[GroupConsole], str]:
        """获取群组列表"""
        return await GroupListUtils.get_group_list(bot)
        ...
        for group in group_list:
            ...
            try:
                result = await bot.send_group_forward_msg(
                    group_id=group.group_id, messages=v11_nodes
                )
    ...
    async def recall_last_broadcast(
        ...
        for group_key, msg_id in msg_ids_to_recall.items():
            try:
                ...
                is_already_deleted = (
                    (retcode == 100 and "MESSAGE_NOT_FOUND" in wording.upper())
                    or (retcode == 300 and "delete message" in wording.lower())
                )
