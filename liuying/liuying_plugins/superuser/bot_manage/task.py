...
async def bot_task(session: Uninfo, bot_id: Match[str] = AlconnaMatch("bot_id")):
    ...
    db_task_list = await TaskInfo.filter().all()
...
async def enable_task(
    ...
    if task_name.available:
        task: TaskInfo | None = await TaskInfo.filter(name=task_name.result).first()
        if not task:
            await MessageUtils.build_message("未找到被动...").finish()
        if bot_id.available:
            logger.info(
                f"开启 {bot_id.result} 被动的 {task_name.result}",
                "bot_manage.task.enable",
                session=session,
            )
            await BotConsole.enable_task(bot_id.result, task.module)
            await MessageUtils.build_message(
                f"已开启 {bot_id.result} 被动的 {task_name.result}"
            ).finish()
        else:
            logger.info(
                f"开启全部 bot 的被动: {task_name.result}",
                "bot_manage.task.enable",
                session=session,
            )
            await BotConsole.enable_task(None, task.module)
            await MessageUtils.build_message(
                f"已开启全部 bot 的被动: {task_name.result}"
            ).finish()
        ...
        bot_id_list = await BotConsole.filter().values_list("bot_id", flat=True)
...
async def disable_task(
    ...
    if task_name.available:
        ...
        if bot_id.available:
            logger.info(
                f"禁用 {bot_id.result} 被动的 {task_name.result}",
                "bot_manage.task.disable",
                session=session,
            )
            await BotConsole.disable_task(bot_id.result, task.module)
            await MessageUtils.build_message(
                f"已禁用 {bot_id.result} 被动的 {task_name.result}"
            ).finish()
        else:
            logger.info(
                f"禁用全部 bot 的被动: {task_name.result}",
                "bot_manage.task.disable",
                session=session,
            )
            await BotConsole.disable_task(None, task.module)
            await MessageUtils.build_message(
                f"已禁用全部 bot 的被动: {task_name.result}"
            ).finish()
    elif bot_id.available:
        logger.info(
