...
def get_group_mute_until(group_id: str) -> float:
    ...
    """
    cached = _mute_state_cache.get(str(group_id))
    if not cached:
        return 0.0
...
async def refresh_bot_group_mute_state(
    ...
    *,
    ...
    cached = _mute_state_cache.get(gid)
    if cached:
    ...
    set_group_mute_until(gid, muted_until, source="member_info")
    return muted_until > now_value
