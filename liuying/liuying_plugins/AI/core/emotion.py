...
from .memory._common import _DEFAULT_PERSONA
...
class EmotionManager:
    ...
    async def update_after_chat(
        ...
        try:
            ...
            current_state = self._state_to_dict(state, thoughts)
    ...
    def _state_to_dict(
        ...
        return {
            ...
            "relation_warmth": float(state.relation_warmth),
    ...
    async def _persist_merged_state(
        ...
        warmth = InnerStateHelper._extract_warmth_scalar(
            merged.get("relation_warmth")
        )
        if warmth is None:
            warmth = 0.3
...
class InnerStateHelper:
    ...
    def merge_state_with_decay(
        ...
        - relation_warmth为标量（0-1），有新值即截断采纳
        ...
        incoming_warmth = InnerStateHelper._extract_warmth_scalar(
            incoming.get("relation_warmth")
        )
        if incoming_warmth is not None:
            merged["relation_warmth"] = (
                InnerStateHelper.clip_relation_warmth(incoming_warmth)
            )
    ...
    @staticmethod
    def _extract_warmth_scalar(value: Any) -> float | None:
        """将关系温暖度归一化为标量

        LLM 返回的 relation_warmth 为 0-1 标量，直接采纳；
        历史遗留数据可能被包装为 {user_id: 分数} 字典，
        取其中最大数值作为代表值。无法解析时返回 None。

        参数:
            value: 标量或历史字典

        返回:
            float | None: 温暖度数值，无有效值时 None
        """
