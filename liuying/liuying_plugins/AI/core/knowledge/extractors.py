...
以及关键词构建与文本分词工具。所有方法均不依赖实例状态，
封装为 KnowledgeExtractor 静态方法，可被 PluginView 等类复用。
"""

import re
from typing import Any, ClassVar

__all__ = ["KnowledgeExtractor", "knowledge_extractor"]


class KnowledgeExtractor:
    """知识库辅助提取器

    封装从插件 extra 字典提取命令/工具/别名、构建检索关键词
    与文本分词的能力，所有方法均为静态方法，通过类变量共享停用词。
    """

    _DEFAULT_KEYWORD_STOPWORDS: ClassVar[set[str]] = {
        "的", "了", "是", "在", "我", "你", "他", "她", "它",
        "请", "帮", "能", "可以", "吗", "么", "啊", "呢", "吧",
    ...
    def extract_commands(
        ...
        for item in raw:
            if isinstance(item, dict):
                cmd: dict[str, Any] = {
                    "command": KnowledgeExtractor.safe_str(
                        item.get("command", "")
                    ),
                    "params": list(item.get("params") or []),
                    "description": KnowledgeExtractor.safe_str(
                ...
                if isinstance(examples_raw, list):
                    for ex in examples_raw:
                        if isinstance(ex, dict):
                            examples.append(
                                {
                                    "exec": KnowledgeExtractor.safe_str(
                                        ex.get("exec", "")
                                    ),
                                    "description": KnowledgeExtractor.safe_str(
                                        ex.get("description", "")
    ...
    def extract_smart_tools(
        ...
        for item in raw:
            if isinstance(item, dict):
                tool: dict[str, Any] = {
                    "name": KnowledgeExtractor.safe_str(
                        item.get("name", "")
                    ),
                    "description": KnowledgeExtractor.safe_str(
                        item.get("description", "")
                ...
                if isinstance(params, dict):
                    tool["parameters"] = {
                        "type": KnowledgeExtractor.safe_str(
    ...
    @staticmethod
    def extract_aliases(extra: dict[str, Any]) -> list[str]:
        """从extra字典提取别名
    ...
    def tokenize(text: str) -> list[str]:
        ...
        stopwords = KnowledgeExtractor._DEFAULT_KEYWORD_STOPWORDS
