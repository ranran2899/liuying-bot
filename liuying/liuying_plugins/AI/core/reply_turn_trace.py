...
def new_trace_id() -> str:
    ...
    返回:
        str: 16位十六进制追踪ID
    """
    return ReplyTurnTrace.new_trace_id()


def current_trace_id() -> str:
    """获取当前上下文的追踪ID（模块级快捷方法）

    返回:
        str: 追踪ID
    """
    return ReplyTurnTrace.current_trace_id()


def set_current_trace_id(
    trace_id: str,
) -> contextvars.ContextVar[str]:
    """设置当前上下文的追踪ID（模块级快捷方法）
    ...
    返回:
        Token: 用于reset的token
    """
    return ReplyTurnTrace.set_current_trace_id(trace_id)
...
def start_trace(
    *,
    ...
    返回:
        str: 追踪ID
    """
    return reply_turn_trace.start_trace(
        trace_id=trace_id,
