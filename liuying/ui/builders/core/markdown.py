...
def _resolve_image_src(content: str | Path, alt: str = "image") -> str:
    """解析图片内容为可用的src路径。"""
...
class MarkdownBuilder(BaseBuilder[MarkdownData]):
    ...
    def _append_element(self, element: MarkdownElement):
        """根据上下文将元素添加到正确的位置。"""
        target = self._context_stack[-1].content if self._context_stack else self._parts
    ...
    def image(self, content: str | Path, alt: str = "image") -> "MarkdownBuilder":
        ...
        self._append_element(
            ImageElement(src=_resolve_image_src(content, alt), alt=alt)
        )
        return self
